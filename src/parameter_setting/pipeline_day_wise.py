...
for session in sessions:
    ...
    for cropping_version in [1]:
        ...
        for k in range(len(init_trial)-1): # run alignment day by day
...
for session in [2, 4]:
    ...
    for cropping_version in [1, 2, 3, 4]:
        # Run equalization
        selected_rows = db.select(states_df, 'equalization', mouse=mouse_number, session=session,
                                  decoding_v=decoding_version,
                                  cropping_v=cropping_version,
                                  motion_correction_v=100,
                                  alignment_v=alignment_version)
        new_selected_rows = main_equalizing(selected_rows, states_df, parameters_equalizer, session_wise=False)
