...
for session in sessions:
    for trial in [1,6,11,16,21]:
        ...
        for cropping_version in cropping_number:
            selected_rows = db.select(states_df, 'component_evaluation', mouse=mouse_number, session=session, trial = trial, is_rest=is_rest,
            ...
            component_evaluation_hdf5_file_path = eval(row['source_extraction_output'])['main']
            ...
            calcium_trace.append(cnm_result[0,:,:])
            calcium_trace_shape.append(cnm_result[0,:,:].shape)
